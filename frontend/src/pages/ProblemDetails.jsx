...
export default function ProblemDetails() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg mt-8">
      {/* Title */}
      <h1 className="text-3xl font-bold mb-2">{problem.title || "Untitled"}</h1>
      ...
      {/* Description */}
      <p className="text-gray-700 mb-6">{problem.description || "No description"}</p>

      {/* Budget & Deadline */}
      <div className="flex flex-wrap gap-6 mb-6">
        <div className="flex items-center space-x-2 text-gray-600">
          <DollarSign className="h-5 w-5" />
          <span className="font-medium">${problem.budget ?? 0}</span>
        </div>
        <div className="flex items-center space-x-2 text-gray-600">
      ...
      {attachments.length > 0 && (
        <div className="mb-6">
          ...
          {attachments.map((file, i) => {
            const cleanFile = file.replace(/^["[\]]+|["\]]+$/g, ""); // remove quotes/brackets
            return (
              <a
                key={i}
              ...
              >
                ...
                <span className="text-gray-700">{cleanFile.split("/").pop()}</span>
              </a>
            );
          })}
        </div>
