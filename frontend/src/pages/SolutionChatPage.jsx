...
export function SolutionChatPage() {
  const { solutionId } = useParams(); // solution-based room
  ...
  useEffect(() => {
    ...
    // Join solution room
    socket.emit("join_room", { roomId: `solution_${solutionId}`, user: userInfo });
  ...
  const sendMessage = () => {
    ...
    const msg = {
      user: user?.fullName || "Me",
      text: input,
      time: new Date().toLocaleTimeString(),
    };

    socket.emit("send_message", { roomId: `solution_${solutionId}`, message: msg });
    setMessages((prev) => [...prev, msg]);
    setInput("");
  };
  ...
  return (
    <div className="max-w-2xl mx-auto py-6">
      <Card className="h-[80vh] flex flex-col">
        <CardHeader>
          <CardTitle>Chat for Solution #{solutionId}</CardTitle>
        </CardHeader>
