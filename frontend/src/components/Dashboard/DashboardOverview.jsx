...
export function DashboardOverview() {
  ...
  const stats = [
    ...
    {
      name: 'Total Earned',
      value: `$${mySolutions.filter(s => s.status === 'approved').reduce((sum, s) => sum + s.payment, 0)}`,
      icon: DollarSign,
      color: 'text-purple-600'
    },
    {
      name: 'Active Projects',
      value: myProblems.filter(p => p.status === 'in_progress').length + mySolutions.filter(s => s.status === 'pending_review').length,
      icon: Clock,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {myProblems.slice(0, 3).map((problem) => (
              <div key={problem.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-500">${problem.budget} • {problem.solutionsCount} solutions</p>
                </div>
            ...
            {myProblems.length === 0 && (
              <p className="text-gray-500 text-center py-4">No problems posted yet</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {mySolutions.slice(0, 3).map((solution) => (
              <div key={solution.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <h4 className="font-medium text-gray-900">{solution.problemTitle}</h4>
                  <p className="text-sm text-gray-500">Submitted {new Date(solution.submittedAt).toLocaleDateString()}</p>
                </div>
                <Badge variant={solution.status === 'approved' ? 'default' : 'secondary'}>
                  {solution.status.replace('_', ' ')}
                </Badge>
              </div>
            ))}
            {mySolutions.length === 0 && (
              <p className="text-gray-500 text-center py-4">No solutions submitted yet</p>
            )}
          </CardContent>
        </Card>
      </div>
